...
static inline uint32_t in32(uint16_t port){uint32_t v;__asm__ __volatile__("inl %1,%0":"=a"(v):"Nd"(port));return v;}
static inline void out32(uint16_t port,uint32_t v){__asm__ __volatile__("outl %0,%1"::"a"(v),"Nd"(port));}
...
static inline uint32_t rd(volatile uint8_t*p,uint32_t off){return *(volatile uint32_t*)(p+off);}static inline void wr(volatile uint8_t*p,uint32_t off,uint32_t v){*(volatile uint32_t*)(p+off)=v;}static inline void wr64(volatile uint8_t*p,uint32_t off,uint64_t v){*(volatile uint64_t*)(p+off)=v;}
static void ring_reset(TRB*ring,size_t*index,uint8_t*cycle){for(size_t i=0;i<MAX_TRBS;i++)ring[i].d[0]=ring[i].d[1]=ring[i].d[2]=ring[i].d[3]=0;ring[MAX_TRBS-1].d[0]=(uint32_t)(uintptr_t)ring;ring[MAX_TRBS-1].d[1]=(uint32_t)((uint64_t)(uintptr_t)ring>>32);ring[MAX_TRBS-1].d[3]=(TRB_LINK<<10)|TRB_TOGGLE_CYCLE|TRB_CYCLE;*index=0;*cycle=1;}
static TRB*ring_next(TRB*ring,size_t*index,uint8_t*cycle){if(*index==MAX_TRBS-1){*index=0;*cycle^=1u;}TRB*t=&ring[*index];t->d[0]=t->d[1]=t->d[2]=t->d[3]=0;++*index;return t;}
...
static int wait_transfer(uint32_t timeout){TRB e;while(timeout--){while(next_event(&e)){if(((e.d[3]>>10)&0x3F)!=TRB_TRANSFER_EVENT)continue;if((uint8_t)(e.d[3]>>24)!=slot_id)continue;if((uint8_t)((e.d[3]>>16)&0x1F)!=ep_id)continue;uint32_t cc=(e.d[2]>>24)&0xFFu;return cc==CC_SUCCESS||cc==CC_SHORT;}__asm__ __volatile__("pause");}return 0;}
...
static void submit_cmd(uint32_t p0,uint32_t p1,uint32_t p2,uint32_t ctl){TRB*t=ring_next(command_ring,&cmd_index,&cmd_cycle);t->d[0]=p0;t->d[1]=p1;t->d[2]=p2;t->d[3]=ctl|(cmd_cycle?TRB_CYCLE:0);db[0]=0;}
...
static uint32_t*ctx(void*context,uint32_t index){return(uint32_t*)((uint8_t*)context+(size_t)index*ctx_size);}
