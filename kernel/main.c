...
static int file_is_ext(const STEVEOS_BOOT_FILE*f,const char*ext){char n[STEVEOS_BOOT_FILE_NAME_MAX];file_name_ascii(f,n,sizeof(n));size_t a=0,b=0;while(n[a])a++;while(ext[b])b++;if(a<b)return 0;for(size_t i=0;i<b;i++){char x=n[a-b+i],y=ext[i];if(x>='A'&&x<='Z')x=(char)(x-'A'+'a');if(y>='A'&&y<='Z')y=(char)(y-'A'+'a');if(x!=y)return 0;}return 1;}
