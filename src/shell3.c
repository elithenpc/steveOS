...
static void click(int x,int y); static void keyboard(EFI_INPUT_KEY k); static void termcmd(void);
...
static void glyph(int x,int y,char c,UINT32 col,int z){const UINT8*q=0;if(c>='A'&&c<='Z')q=L[c-'A'];else if(c>='a'&&c<='z')q=L[c-'a'];else if(c>='0'&&c<='9')q=N[c-'0'];if(q){for(int a=0;a<5;a++)for(int b=0;b<7;b++)if(q[a]&(1u<<b))rr((RECT){x+a*z,y+b*z,z,z},col);return;}if(c=='.')rr((RECT){x,y+6*z,z,z},col);else if(c=='-')rr((RECT){x,y+3*z,5*z,z},col);else if(c==':'){rr((RECT){x,y+2*z,z,z},col);rr((RECT){x,y+6*z,z,z},col);}else if(c=='_')rr((RECT){x,y+6*z,5*z,z},col);else if(c=='>'){rr((RECT){x,y+2*z,z,z},col);rr((RECT){x+z,y+3*z,z,z},col);rr((RECT){x,y+4*z,z,z},col);}}
...
static void draw_net(WIN*w){RECT c={w->x,w->y+30,w->w,w->h-30};rr(c,panelc());txt(c.x+18,c.y+18,"NETWORK",fgc(),2);txt(c.x+18,c.y+60,"NIC",subc(),1);txt(c.x+170,c.y+60,"DEFERRED",fgc(),1);txt(c.x+18,c.y+94,"TCP IP",subc(),1);txt(c.x+170,c.y+94,"NATIVE LAYER PENDING",fgc(),1);txt(c.x+18,c.y+128,"DNS",subc(),1);txt(c.x+170,c.y+128,"NATIVE LAYER PENDING",fgc(),1);}
...
static void desktop(void){
    ...
    txt(306,210,"DRIVER DEFERRED",fgc(),2);
    txt(306,246,"SAFE BOOT MODE",subc(),1);
    for(int i=0;i<9;i++)
        rr((RECT){30+i*((int)sc.w/9),294,(int)sc.w/18,1},0x202D42);
...
static void menu(void){int h=55+8*36+10;RECT m={18,(int)sc.h-h-10,440,h};roundr((RECT){m.x+7,m.y+7,m.w,m.h},0x050811,12);roundr(m,panelc(),12);rr((RECT){m.x,m.y,m.w,44},barc());txt(m.x+16,m.y+13,"STEVEOS",fgc(),2);const char*a[]={"TERMINAL","TASK MANAGER","FILES","SYSTEM INFO","NETWORK","BLEHHH VIEWER","SETTINGS","INSTALLER"};for(int i=0;i<8;i++){RECT q={m.x+10,m.y+50+i*36,m.w-20,29};if(i==start_pick)roundr(q,acc(),8);txt(q.x+12,q.y+8,a[i],fgc(),1);}}
...
static void keyboard(EFI_INPUT_KEY k){if(start_open){if(k.ScanCode==SCAN_UP){start_pick--;if(start_pick<0)start_pick=7;redraw();return;}if(k.ScanCode==SCAN_DOWN){start_pick++;if(start_pick>7)start_pick=0;redraw();return;}if(k.UnicodeChar==CHAR_CARRIAGE_RETURN){launch(start_pick);return;}if(k.ScanCode==SCAN_ESC){start_open=0;redraw();return;}}if(active>=0&&wins[active].open&&wins[active].id==W_TERM){if(k.UnicodeChar==CHAR_BACKSPACE){if(input_len)input[--input_len]=0;redraw();return;}if(k.UnicodeChar==CHAR_CARRIAGE_RETURN){termcmd();redraw();return;}if(k.UnicodeChar>=32&&k.UnicodeChar<127&&input_len<89){CHAR8 c=(CHAR8)k.UnicodeChar;if(c>='a'&&c<='z')c-=32;input[input_len++]=c;input[input_len]=0;redraw();return;}}if(k.UnicodeChar=='s'||k.UnicodeChar=='S'){start_open=!start_open;start_pick=0;redraw();return;}if(k.UnicodeChar=='t'||k.UnicodeChar=='T'){launch(W_TERM);return;}if(k.UnicodeChar=='k'||k.UnicodeChar=='K'){launch(W_TASKS);return;}if(k.UnicodeChar=='f'||k.UnicodeChar=='F'){launch(W_FILES);return;}if(k.ScanCode==SCAN_ESC&&active>=0){wins[active].open=0;active=-1;redraw();}}
